...
/**
 * Created by Regis Gaboardi (@gmail.com) Provided with Love and IntelliJ IDEA
 * for SistemaDePedidosWeb. 09/10/2020
 */
...
public class PedidoDAO {
	...
	/**
	 * Retorna o Pedido completo (Pedido, Cliente, ItensDoPedido (Produto,
	 * Quantidade)) de um Cliente.
	 * 
	 * @param clientID
	 * @return
	 * @throws SQLException
	...
	public List<ItemDoPedido> findItensDoCliente(int clientID) throws SQLException {
		...
		String findItens = "SELECT prd.id AS proid, prd.descricao AS prodesc, pp.quantidade as quantidade "
				+ "FROM produto_pedido pp " + "JOIN produto prd ON prd.id = pp.produto_fk "
				+ "JOIN pedido pdd ON pp.pedido_fk = pdd.id " + "WHERE pdd.cliente_fk = ?";
		try {
	...
	public void atualizaQuantidade(ItemDoPedido item, int cliente) throws SQLException {
		this.connection = new ConnectionFactory().getConnection();
		String update = "UPDATE produto_pedido pp " + "JOIN produto pdt ON pp.produto_fk = pdt.id "
				+ "JOIN pedido pdd ON pp.pedido_fk = pdd.id " + "SET pp.quantidade = ? " + "WHERE pdt.descricao = ? "
				+ "AND pdd.cliente_fk = ?";
		try {
	...
	/**
	 * Varre todos os Pedidos e DELETA todos os pedidos em que a quantidade de TODOS
	 * os seus itens � menor ou igual a ZERO.
	 * 
	 * @param cliente
	 * @throws SQLException
	 */
	public void deleteGarbage(int cliente) throws SQLException {
		this.connection = new ConnectionFactory().getConnection();
		String delete = "DELETE pp FROM produto_pedido pp " + "JOIN pedido pdd ON pp.pedido_fk = pdd.id "
				+ "WHERE 0 >= pp.quantidade " + "AND pdd.cliente_fk = ?";
		try {
	...
	public List<Pedido> fetchAll() throws Exception {
		...
		String findAll = "" + "SELECT " + "p.id as pid, " + "p.data AS data, " + "c.id AS clienteId, "
				+ "c.nome as nomeCliente, " + "c.sobrenome as sobrenome, " + "c.cpf as cpf,"
				+ "SUM(pp.quantidade) as quantidade " + "FROM pedido p " + "JOIN cliente c ON p.cliente_fk = c.id "
				+ "JOIN produto_pedido pp on p.id = pp.pedido_fk " + "JOIN produto prd on pp.produto_fk = prd.id "
				+ "GROUP BY pid ORDER BY p.id DESC";
		try {
	...
	public Pedido find(int id) throws SQLException {
		this.connection = new ConnectionFactory().getConnection();
		Pedido result = new Pedido();
		String findOne = "SELECT " + "p.data AS data," + "c.id AS cid, c.nome, c.sobrenome, c.cpf,"
				+ "prd.id AS proid, prd.descricao AS prodesc," + "pp.quantidade " + "FROM pedido p "
				+ "JOIN cliente c ON p.cliente_fk = c.id " + "JOIN produto_pedido pp on p.id = pp.pedido_fk "
				+ "JOIN produto prd on pp.produto_fk = prd.id " + "WHERE p.id = ?";
		try {
			PreparedStatement ps = connection.prepareStatement(findOne, ResultSet.TYPE_SCROLL_SENSITIVE,
	...
	public Pedido salvarPedido(Pedido pedido) {
		...
		try {
			...
			sql.append("insert into pedido (`data`, `cliente_fk`) values (?, ?)");
			...
			preparedStmt.setInt(2, pedido.getCliente().getId());
	...
	public Integer findByUltimoId() {

		try {

			StringBuffer sql = new StringBuffer();

			sql.append("select id from pedido order by id desc limit 1");

			this.connection = new ConnectionFactory().getConnection();

			PreparedStatement itensStatement = connection.prepareStatement(sql.toString());
	...
	public void salvarItem(ItemDoPedido item, Integer idPedido) {
		...
		try {
			...
			sql.append("insert produto_pedido (`produto_fk`, `pedido_fk`, `quantidade`) values (?, ?, ?);");

			PreparedStatement preparedStmt = connection.prepareStatement(sql.toString());

			preparedStmt.setInt(1, item.getProduto().getId());
			preparedStmt.setInt(2, idPedido);
			preparedStmt.setInt(3, item.getQuantidade());
